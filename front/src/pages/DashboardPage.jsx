    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">High-level overview of office equipment.</p>
        </div>
        <Link 
          to="/devices" 
          className="rounded-xl border border-[#30363d] bg-[#0d1117] px-4 py-2 text-sm font-semibold text-slate-200 hover:bg-[#21262d] hover:text-white transition-colors"
        >
          Open devices
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {cards.map((card) => (
          <div key={card.label} className="overflow-hidden rounded-2xl border border-[#2a323d] bg-[#1a1f26]">
            <div className="p-5">
