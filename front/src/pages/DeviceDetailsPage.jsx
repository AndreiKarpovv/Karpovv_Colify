    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Device details and activity notes</p>
        </div>
        <Link 
          to="/devices" 
      ...
      <div className="grid grid-cols-1 gap-4 rounded-2xl border border-[#2a323d] bg-[#1a1f26] p-5 md:grid-cols-2">
      ...
      <section className="rounded-2xl border border-[#2a323d] bg-[#1a1f26] p-5">
        <h3 className="text-lg font-semibold text-white">Device notes</h3>
        <p className="mb-4 text-sm text-slate-400">Workers can add notes about usage, issues, and context.</p>
        ...
        <form onSubmit={handleAddNote} className="mb-5 space-y-3">
          <textarea
            ...
            className="w-full rounded-xl border border-[#30363d] bg-[#0d1117] px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:border-amber-500 focus:outline-none transition-colors"
          />
          <button
            type="submit"
        ...
        {notes.length === 0 ? (
          ...
          <div className="space-y-3">
            {notes.map((note) => {
              ...
              return (
                <div key={note.id} className="rounded-xl border border-[#2a323d] bg-[#161b22] p-3">
                  <div className="mb-1 flex items-center justify-between gap-3">
                    <p className="text-sm font-semibold text-white">
                      {note.expand?.author?.name || note.expand?.author?.email || 'Unknown'}
                    </p>
                    <div className="flex items-center gap-3">
                      <p className="text-xs text-slate-400">{new Date(note.created).toLocaleString()}</p>
                      {canDelete ? (
                        <button
                          type="button"
                  ...
                  <p className="whitespace-pre-wrap text-sm text-slate-300 mt-1">{note.text}</p>
                </div>
              );
            })}
          </div>
        ...
        {error ? (
          <p className="mt-3 rounded-lg bg-rose-950/20 border border-rose-900/50 px-3 py-2 text-sm text-rose-300">
            {error}
          </p>
        ) : null}
      </section>
